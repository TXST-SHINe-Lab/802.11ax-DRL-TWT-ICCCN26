...
def main() -> None:
    ...
    if not npz_path.exists():
        print(
            f"ERROR: {npz_path} not found; constants can only be derived from a stacked EDA run. "
            f"Nothing written. Run 3-prepare-data.sh for this run, or use the existing "
            f"derived_constants.json.",
            file=sys.stderr,
        )
        sys.exit(1)
