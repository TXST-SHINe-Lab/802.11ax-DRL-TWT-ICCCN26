...
"""
pb_twt_wrapper_py.py - Python-side TWT wrapper for the NS-3 simulation.

Hides every NS-3 and NS3-AI detail behind a small Python API, so a controller,
whether an RL agent or a heuristic, sees only plain Python dicts.

Author: Ahmed Maksud <ahmed.maksud@email.ucr.edu>
Lab: SHINE Lab, Texas State University

Usage:
    from pb_twt_wrapper_py import TWTWrapper, create_default_action
...
class TWTWrapper:
    ...
    def _import_bindings(self):
        ...
        try:
            import pb_twt_interface_py as py_binding

            self.py_binding = py_binding
            self.MAX_NUM_STA = py_binding.MAX_NUM_STA
            self.MAX_NUM_TWT_GROUPS = py_binding.MAX_NUM_TWT_GROUPS
            self._log("✓ Imported pb_twt_interface_py", "green")
        except ImportError as e:
            raise RuntimeError(
                f"Failed to import pb_twt_interface_py: {e}\n"
    ...
    def _env_struct_to_dict(self, env) -> Dict:
        ...
        for i in range(env.num_sta):
            ...
            realistic = {
                ...
                "nominal_msdu_size": r.nominal_msdu_size,
                "mean_data_rate_kbps": r.mean_data_rate_kbps,
                "delay_bound_ms": r.delay_bound_ms,
    ...
    def reset(self, seed: int = 9000) -> Dict:
        """
        ...
        try:
            ...
            self.msgInterface = self.exp.run(
                ...
                show_output=self.verbose,  # Only show NS-3 output if verbose mode
            )
    ...
    def close(self):
        ...
        # Kill the subprocess and release the shared memory.
        # Without it the next reset() fails with boost::interprocess_exception::library_error.
