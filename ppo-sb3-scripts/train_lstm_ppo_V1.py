...
"""train_lstm_ppo_V1.py - LSTM-PPO V1 training script for TWT WiFi scheduling.

Trains a recurrent PPO agent with sb3-contrib's RecurrentPPO on the TWT Gymnasium environment.
See train_ppo_V1.py for the feed-forward variant; the two scripts are otherwise parallel.
...
Usage:
    python3.11 train_lstm_ppo_V1.py --total-timesteps 100000 --seed 42
    python3.11 train_lstm_ppo_V1.py --resume checkpoints/<run_id>/lstm_ppo_V1_twt_<preset>_<timestamp>/lstm_ppo_V1_twt_final.zip
...
# file_comm_env.py and reward_functions.py sit beside this script in ppo-sb3-scripts/.
_script_dir = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, _script_dir)

# RecurrentPPO lives in sb3-contrib, not stable-baselines3.
try:
    from sb3_contrib import (
        RecurrentPPO as PPO,
    )  # Aliased to PPO so the rest of the file matches train_ppo_V1.py
    ...
    print(f"Error: sb3-contrib not installed. Run: pip install sb3-contrib")
...
DEFAULT_PPO_HYPERPARAMS = {
    # Core RecurrentPPO params
    "learning_rate": 2.5e-4,  # Below the 3e-4 of the MLP variant, for LSTM stability
    "n_steps": 128,  # Steps per rollout, should be at least the episode length
    "batch_size": 64,  # Minibatch size, must divide n_steps
    "n_epochs": 10,  # Passes over each rollout
    "gamma": 0.99,  # Discount factor
    "gae_lambda": 0.95,  # GAE lambda
    "clip_range": 0.2,  # PPO clip range
    "clip_range_vf": None,  # None disables value-function clipping
    "normalize_advantage": True,
    # Entropy and value function
    "ent_coef": 0.01,  # Entropy coefficient, drives exploration
    "vf_coef": 0.5,  # Value function coefficient
    "max_grad_norm": 0.5,  # Gradient clipping
    # LSTM architecture
    "policy_kwargs": {
        "lstm_hidden_size": 256,  # Hidden state width
        "n_lstm_layers": 1,  # Number of LSTM layers
        "shared_lstm": False,  # Policy and value each get their own LSTM
        "enable_critic_lstm": True,  # Required when shared_lstm is False
        "net_arch": dict(
            pi=[256, 128],  # MLP head after the LSTM
            vf=[256, 128],  # MLP head after the LSTM
        ),
    },
}
                ...
                if self.episode_count % self.log_freq == 0:
                    ...
                    log_entry = {
                        ...
                        "avg_queue_bytes": terminal_info.get(
                            "avg_queue_bytes", 0
                        ),  # Latency proxy (avg_latency_ms always 0)
...
class RewardLoggerCallback(BaseCallback):
    ...
    def __init__(
        ...
        log_dir: str,
        run_timestamp: str,
        save_freq: int = 10000,  # Save every N timesteps
        verbose: int = 0,
    ):
        super().__init__(verbose)
        self.log_dir = log_dir
...
def train_ppo(
    ...
    # PPO hyperparameters (can override defaults)
    ...
    net_arch_pi: list = [256, 128],
    net_arch_vf: list = [256, 128],
    # Output config
    ...
    """
    Train a recurrent PPO agent on the TWT environment.
    ...
    Args:
        ...
        net_arch_pi: Policy network architecture, the MLP head after the LSTM
        net_arch_vf: Value network architecture, the MLP head after the LSTM
    ...
    run_name = f"lstm_ppo_V1_twt_{reward_preset}_{timestamp}"
    ...
    print("=" * 60)
    print("LSTM-PPO V1 Training for TWT WiFi Scheduling")
    print("=" * 60)
    ...
    # Build the LSTM policy kwargs; these mirror DEFAULT_PPO_HYPERPARAMS above.
    policy_kwargs = {
        "lstm_hidden_size": 256,  # Hidden state width
        "n_lstm_layers": 1,  # Number of LSTM layers
        "shared_lstm": False,  # Policy and value each get their own LSTM
    ...
    if resume_path and os.path.exists(resume_path):
        ...
        print("\nCreating new LSTM-PPO V1 model...")
        model = PPO(
            policy="MlpLstmPolicy",  # The recurrent counterpart of train_ppo_V1.py's MlpPolicy
            env=vec_env,
    ...
    # Save final model
    final_path = os.path.join(checkpoint_dir, "lstm_ppo_V1_twt_final.zip")
