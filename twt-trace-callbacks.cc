 ...
 * @brief NS-3 trace callback implementations for PHY, MAC, and energy tracking
 */
...
// Defaults to 0 here and is overwritten in twt-main-simulation.cc from config.keepTrackOfMetricsFrom_ms.
double keepTrackOfMetricsFrom_ms = 0; // ms, callbacks discard samples before this time
...
uint64_t* lastRxTimestampUsForSta = nullptr;   // Last RX time from this STA in us, 0 if none yet
...
// By cross-referencing packet UIDs across events, you can reconstruct complete
// packet paths and identify where/when packets are lost or delayed.
//
...
E2E_MacEnqueue(std::string context, Ptr<const Packet> packet)
{
    ...
    if (Simulator::Now().GetMilliSeconds() >= keepTrackOfMetricsFrom_ms)
    {
        uint32_t uid = packet->GetUid();
        e2eTraceFile << std::fixed << std::setprecision(6) << Simulator::Now().GetMilliSeconds()
                     << ",MAC_ENQUEUE,STA" << (nodeId - 1) << "," << uid << "," << packet->GetSize()
                     << "," << std::endl;
    }
}
...
// Note: oldSize parameter is provided by NS-3 but not used here since we only care
// about the current queue state. Use (newSize - oldSize) to track queue change deltas.
void
...
// This allows analysis of both single-packet (nMpdus=1) and aggregated (nMpdus>1)
// transmissions, providing visibility into WiFi 6 A-MPDU aggregation behavior.
// Note: txPowerW is a required NS-3 callback parameter (in Watts)
void
    ...
    if (Simulator::Now().GetMilliSeconds() >= keepTrackOfMetricsFrom_ms)
    {
        ...
        for (const auto& pair : psduMap)
        {
            ...
            if (airtimeUsedUsForSta)
            {
                ...
                Time ppduDuration =
                    WifiPhy::CalculateTxDuration(totalBytes, txVector, WIFI_PHY_BAND_5GHZ);
            ...
            ampduTraceFile << std::fixed << std::setprecision(6)
                           ...
                           << nMpdus << "," << totalBytes << "," << std::setprecision(3) << txPowerW
                           << std::endl;
        }
    }
}
...
// Fired whenever the WiFi PHY transitions to a new state. The callback receives
// the previous state (state parameter) and the duration it was active (duration).
// Example: If radio was in IDLE for 100µs then transitions to TX, callback fires
// with state=IDLE, duration=100µs. This allows tracking total time spent in each
// state across the simulation for energy consumption calculations.
// Energy is computed as: (state_current_mA × state_duration_ms) and accumulated
// per STA to derive total battery consumption at end of simulation.
void
...
// AP sends PSDU → waits for ACK → if ACK doesn't arrive within timeout window,
// this callback fires and increments the aggregate downlink timeout counter.
...
// 1. EXPIRED_LIFETIME: Packet sat in queue too long (TTL exceeded). Indicates queue
//    congestion or slow transmission rate. Packet validity expired before transmission.
// 2. FAILED_ENQUEUE: Queue was full (MAX_QUEUE_SIZE_BYTES exceeded), new packet rejected.
//    Indicates insufficient buffer capacity or backlog.
//
// Difference from PSDU timeouts: MPDU drops track individual packet rejections/expiries
// before or after queuing, while PSDU timeouts track frame-level transmission failures
// (missing ACKs). A packet may be dropped as MPDU before it's even assembled into a PSDU.
void
...
// Signature: (Ptr<const Packet>, uint16_t channelFreqMhz, WifiTxVector, MpduInfo, SignalNoiseDbm,
// uint16_t staId) Populates: rx_fragment_count, RSSI/SNR, MCS, NSS, channel width, etc.
void
...
// Realistic metric. AP counts RX failures. Uses Ptr<const Packet> signature.
void
...
// Realistic metric. AP directly counts RX failures. However, for dropped frames
// we often cannot identify the source STA (frame is corrupted).
...
// Oracle metric. This callback fires at the STA's MAC layer when STA successfully
// transmits (receives ACK from AP). This is STA-side local counter (dot11TransmittedFragmentCount).
...
// Oracle metric. This callback fires at the STA's MAC layer when STA's transmission
// fails (max retries exceeded). This is STA-side local counter (dot11FailedCount).
// AP cannot directly observe this - would require 802.11k STA Statistics Request/Response.
// Kept as ORACLE for ground-truth analysis and RL training.
void
...
Initialize802dot11kArrays(uint32_t numSta)
{
    ...
    lastRxTimestampUsForSta = new uint64_t[numSta]();
    ...
    // Initialize with NAN for double arrays (no valid measurement yet)
    // and 0 for integer counters. NAN indicates "no data received" which
    // will be converted to INT8_MIN (-128) sentinel in the struct.
...
// Realistic metric. This is 802.11ax BSR protocol data. STA explicitly sends BSR to AP
// (in Trigger Frame response or QoS Data frame). AP receives and parses BSR from
// frame payload - this is the actual protocol exchange in action.
...
{
    std::string e2eTraceFileName =
        "contrib/ai/examples/twt/data-log/ns3-e2e-trace-" + simIdString + ".csv";
    ...
    std::string macQueueSizeTraceFileName =
        "contrib/ai/examples/twt/data-log/ns3-macqueuesize-trace-" + simIdString + ".csv";
    ...
    std::string ampduTraceFileName =
        "contrib/ai/examples/twt/data-log/ns3-ampdu-trace-" + simIdString + ".csv";
    ampduTraceFile.open(ampduTraceFileName);
    ampduTraceFile << "Time_ms,Node,NumMPDUs,TotalBytes,TxPower_W" << std::endl;

    std::string bsrTraceFileName =
        "contrib/ai/examples/twt/data-log/ns3-bsr-trace-" + simIdString + ".csv";
    bsrTraceFile.open(bsrTraceFileName);
    if (bsrTraceFile.is_open())
    {
        // Note: BSR uses MAC address instead of STA ID because it's fired at MAC layer
        // where only the MAC address is available (no context string to extract node ID)
        bsrTraceFile << "Time_ms,StaMacAddress,TID,QueueSizeBytes,QueueSizeUnits" << std::endl;
    }

    std::string phyStateTraceFileName =
        "contrib/ai/examples/twt/data-log/ns3-phystate-trace-" + simIdString + ".csv";
    ...
    std::string txRxStatsTraceFileName =
        "contrib/ai/examples/twt/data-log/ns3-txrx-stats-trace-" + simIdString + ".csv";
    ...
    std::string linkMeasurementTraceFileName =
        "contrib/ai/examples/twt/data-log/ns3-link-measurement-trace-" + simIdString + ".csv";
    ...
    std::string timeoutDropTraceFileName =
        "contrib/ai/examples/twt/data-log/ns3-timeout-drop-trace-" + simIdString + ".csv";
...
// Each trace callback function defined above needs to be "hooked" to the actual trigger
// that fires it. NS-3 provides these triggers as callback hooks. Config::Connect() does
// this registration: it links a callback function to a trace source so that when the
// trigger fires, the callback gets invoked automatically with event data.
...
ConnectSummaryTraces(NodeContainer staNodes)
{
    for (std::size_t ii = 0; ii < staNodes.GetN(); ii++)
    {
        ...
        // Try to connect to OnOffApplication (Voice Assistant, Video Streaming)
        ...
        {
            Config::Connect(onoffTxStr.str(), MakeCallback(&TxTraceAtApp));
        }

        // Try to connect to UdpClient (IoT Sensors, Video Cameras)
        ...
        {
            Config::Connect(udpTxStr.str(), MakeCallback(&TxTraceAtApp));
        }
